...
#define E_DUPLICATE             (-4)                /* 重复项或重复执行 */
#define E_NONE_EXISTS           (-5)                /* 访问项不存在 */
#define E_NONE_CONTEXT          (-6)                /* 不具备可运行的状态。常见的就是没有初始化或启动 */
#define E_NONE_RELEASED         (-7)                /* 当前（资源）项未被释放，无法被析构或再次分配 */
#define E_OUT_OF_RANGE          (-8)                /* 请求的范围超出限制 */
#define E_OUT_OF_MEMORY         (-9)                /* 内存不足（最基本的资源不足） */
#define E_OUT_OF_SUPPLY         (-10)               /* （通用的）请求数量大于供给能力，或者数据下溢（underflow） */
#define E_OUT_OF_CAPACITY       (-11)               /* （通用的）数据量超出设计容量；或者数据溢出（overflow） */
#define E_BUSY                  (-12)               /* 当前无法运行或答复，需要稍后重试 */
#define E_TIMEOUT               (-13)               /* 超时 */
#define E_NO_PERMISSION         (-14)               /* 无权限访问 */
#define E_NO_SUPPORT            (-15)               /* 程序不支持（未设计）该功能 */
